...
static CGFloat const kStep1Duration = 1.0;//执行时间
static CGFloat const kStep2Duration = 0.5;//第二步动画的执行时间
static CGFloat const kStep3Duration = 0.2;//第三步动画的执行时间
static CGFloat const kStep4Duration = 0.25;//第四步动画的执行时间
static CGFloat const kStep5Duration = 0.5;//第五步动画的执行时间
...
static CGFloat const kRadius = 40;//圆的半径
static CGFloat const kLineWidth = 6;//线条宽度
static CGFloat const kVerticalThinLayerWidth = 3;//开始进入圆内前线条的宽度
static CGFloat const kVerticalMoveLayerHeight = 15;//开始进入圆内前线条的长度
static CGFloat const kYScale = 0.8;
static CGFloat const kVerticalFatLayerWidth = 6;//已进入圆内部时线条的宽度

@interface OneLoadingAnimationView ()
@property (nonatomic) BOOL isSuccess;///<是否加载成功

@property (nonatomic) ArcTocircleLayer *arcToCircleLayer;///<动画layer
@property (nonatomic) CAShapeLayer *moveArcLayer;///<这个类的作用主要是沿着类涂色
@property (nonatomic) CALayer *verticalMoveLayer;///<第三步动画layer
@property (nonatomic) CAShapeLayer *verticalDisappearLayer;
@property (nonatomic) CAShapeLayer *verticalAppearLayer;

@property (nonatomic) CAShapeLayer *leftAppearLayer;///<第五部分动画左侧条
@property (nonatomic) CAShapeLayer *rightAppearLayer;///<第五部分动画右侧条
@property (nonatomic) CAShapeLayer *checkMarkLayer;///<核对无误对号图层
@property (nonatomic) CAShapeLayer *exclamationMarkTopLayer;///<感叹号的上半部分
@property (nonatomic) CAShapeLayer *exclamationMarkBottomLayer;///<感叹号的下半部分
...
- (void)doStep5b
{
    ...
    // animation
    CABasicAnimation *startAnimation = [CABasicAnimation animationWithKeyPath:@"strokeStart"];
    startAnimation.fromValue = @(SSForm);
    startAnimation.toValue = @(SSTo);
    
    CABasicAnimation *endAnimation = [CABasicAnimation animationWithKeyPath:@"strokeEnd"];
    endAnimation.fromValue = @(SEForm);
    endAnimation.toValue = @(SETo);
